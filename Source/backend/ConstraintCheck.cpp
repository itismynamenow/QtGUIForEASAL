...
ConstraintCheck::ConstraintCheck(ActiveConstraintGraph* cgk, PredefinedInteractions *df) {
...
void ConstraintCheck::updatePossibleContactList(Atom* a, Atom* b, int indexA, int indexB){
	double* aLoc = a->getLocation();
	double* bLoc = b->getLocation();
	double originDist = sqrt(((aLoc[0]-bLoc[0])*(aLoc[0]-bLoc[0]))+
					   ((aLoc[1]-bLoc[1])*(aLoc[1]-bLoc[1]))+
	...
	if( bondingLowerBound <= originDist   &&   originDist  <= bondingUpperBound)
			(this->contactMap[this->flip])[ abs(originDist- middle) ] = make_pair(indexA,indexB);
}
...
bool ConstraintCheck::angleViolationCheck( double fb[3][3], double tb[3][3])
{
   ...
   Vector3d TB = t + mean;
	...
	if(angle < Settings::Constraint::angleLow  || angle > Settings::Constraint::angleHigh)
		return true;
	//-----
...
//	if(! (TB(0)>-Settings::Sampling::GridXY && TB(0)<Settings::Sampling::GridXY   &&   TB(1)>-Settings::Sampling::GridXY && TB(1)<Settings::Sampling::GridXY   &&   TB(2)>-Settings::Sampling::GridZ && TB(2)<Settings::Sampling::GridZ)   ) //not inGrid
//		return true;
...
//	if(!Settings::Constraint::stericConstraint)
//			return false; // to remove collision constraint for test case
	...
	//todo angleViolationCheck and getXFAtoms may both be computing the orientation i.e. transformation. clean the redundancy

	int indexA = 0;


	for(map<double,pair<int,int> >::iterator iter = oldContacts.begin(); iter != oldContacts.end() && !out; iter++){
	...
	if(!out){
		...
		for(vector<Atom*>::iterator ita = first.begin(); 	ita != first.end() && !out;   ita++)
		{
			int indexB = 0;

			for(vector<Atom*>::iterator itb = second.begin();
					itb != second.end();itb++)
			{
	...
	for(vector<Atom*>::iterator ita = second.begin();
			ita != second.end();ita++)
	{
		delete *ita;
	}
	...
	if(Settings::Constraint::wholeCollision ){ // to consider all atoms not just interface atoms. if candidate_interactions is provided, then molecular unit consisted of only interface atoms
		...
		bool is_nei_collide = is_collide(this->helA->getStree(),this->helA->getStree(),trans_b,this->nei_matrix);
...
list<pair<int,int> > ConstraintCheck::getContacts(int flipno){
	...
	for(iter = this->contactMap[flipno].begin();iter != this->contactMap[flipno].end();iter++){
