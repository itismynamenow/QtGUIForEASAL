...
/*
Orientation* CartesianRealizer::computeRealization(ActiveConstraintGraph* cgk, ConvexChart* descr, int flipno, bool& fail ) {

	return computeRealization(cgk, descr, flipno, descr->param_length, fail );
}


Orientation* CartesianRealizer::computeRealization(ActiveConstraintGraph* cgk, ConvexChart* descr, int flipno,  double paramconnections[][12],  bool& fail  ) {
	fail = false;
	...
	for(int i= 0;i<NO;i++){
		positions[i][0]= -111;	positions[i][1]= -111;	positions[i][2]= -111;
	}
	...
	if( partial3tree){
		...
		for(int i = 0; i<3; i++){
			for(int j = 0; j<3; j++){
				...
				if(toA[i][j]== -111 || toB[i][j]== -111 || std::isnan(toA[i][j])  || std::isnan(toB[i][j]) )
		...
		for(int j=0; j<3; j++){ //tranlates toB[j]
			double pot[3];
			Utils::matApp(toB[j] ,toA[0], toA[1], toA[2], fromA[0], fromA[1], fromA[2], pot);
...
Orientation* CartesianRealizer::computeRealization(ActiveConstraintGraph* cgk,
        ...
        bool& fail, vector<vector<int> > flipScheme) {
    fail = false;
...
bool CartesianRealizer::isDistorted()
{
    ...
    double *p1 = new double[3]; p1[0] = 0;  p1[1] = 0;  p1[2] = 0;
    double *p2 = new double[3]; p2[0] = 0;  p2[1] = 0;  p2[2] = 1;
...
bool CartesianRealizer::locateVertex(int tetrahedron_index,  bool mirror){
	...
	if( ind[0] <NO/2 && ind[1] <NO/2 && ind[2]<NO/2 && ind[3] <NO/2  ) //inside of same helix i.e. helix A
	{
		double *from[4];
		for(int i=0; i<4; i++ ){
		...
		double pot[3];
		Utils::matApp(from[0], from[1], from[2], from[3], positions[ ind[1] ], positions[ ind[2] ], positions[ ind[3] ], pot);
	...
	else if( ind[0] >= NO/2 && ind[1] >= NO/2 && ind[2] >= NO/2 && ind[3] >= NO/2)  //inside of same helix i.e. helix B
	{
		double *from[4];
		for(int i=0; i<4; i++){
...
//	debug = false;
	double pot[3];
	Utils::matApp(posi[0], posi[1], posi[2], posi[3], positions[ ind[1] ], positions[ ind[2] ], positions[ ind[3] ], pot);
	...
	if(debug) cout << ind[0] << "-" << ind[1] << "   " << ind[0] << "-" << ind[2] << "   " << ind[0] << "-" << ind[3] << "   " << endl;
	if(debug) cout << "fact " << edge_length[ind[0]][ind[1]] << " "<< edge_length[ind[0]][ind[2]] << " "<< edge_length[ind[0]][ind[3]] << endl;
	if(debug) cout << "before "<< Utils::dist(posi[0], posi[1]) << " "<< Utils::dist(posi[0], posi[2]) << " "<< Utils::dist(posi[0], posi[3]) << endl;
	if(debug) cout << "later "<< Utils::dist(positions[ ind[0] ],positions[ ind[1] ]) << " "<< Utils::dist(positions[ ind[0] ], positions[ ind[2] ]) << " "<< Utils::dist(positions[ ind[0] ], positions[ ind[3] ]) << endl;
	...
	for(int i=0; i<4; i++)
		if(std::isnan(positions[ind[i]][0]) || std::isnan(positions[ind[i]][1]) || std::isnan(positions[ind[i]][2]) )
			foundNan = true;
	...
	for(int i=0; i<4; i++)
		for(int j=i+1; j<4; j++)
			if( abs(Utils::dist(posi[i], posi[j])-Utils::dist(positions[ ind[i] ],positions[ ind[j] ])) > .6 )
				disturbed = true;
	...
	if( disturbed || foundNan)
	{
		...
		if(debug)
		for(int i=1; i<4; i++){
			cout << ind[i] << ": " << positions[ind[i]][0] << " " << positions[ind[i]][1] << " " << positions[ind[i]][2] << endl;
			for(int j=i+1; j<4; j++)
				cout << ind[i] << "-" << ind[j] << ": " << edge_length[ind[i]][ind[j]] << " " <<  Utils::dist(positions[ ind[i] ],positions[ ind[j] ]) << endl;
		}


		double * pot = Utils::find4thPoint_inTheSamePlane(
							positions[ ind[1] ], positions[ ind[2] ], positions[ ind[3] ],
							edge_length[ ind[0] ][ ind[1] ], edge_length[ ind[0] ][ ind[2] ], edge_length[ ind[0] ][ ind[3] ] );

		for(int i=0; i<3; i++)
			positions[ ind[0] ][i] = pot[i];

		if(debug) cout << ind[0] << ": " << positions[ind[0]][0] << " " << positions[ind[0]][1] << " " << positions[ind[0]][2] << endl;

		delete[] pot;

		if(std::isnan( positions[ ind[0] ][0] ) || std::isnan( positions[ ind[0] ][1] ) || std::isnan( positions[ ind[0] ][2] )  ) //if volume is close to 0, even if locateTetrahedron gives correct numbers after matApp, they become nan. matApp doesnot work for volume 0 case. It cannot compute translation matrix for this case!
				realizable=false;
		...
		if( abs(edge_length[ind[0]][ind[1]]-Utils::dist(positions[ ind[0] ],positions[ ind[1] ])) > .6)
			realizable=false;
		if( abs(edge_length[ind[0]][ind[2]]-Utils::dist(positions[ ind[0] ],positions[ ind[2] ])) > .6)
			realizable=false;
		if( abs(edge_length[ind[0]][ind[3]]-Utils::dist(positions[ ind[0] ],positions[ ind[3] ])) > .6)
			realizable=false;
...
void CartesianRealizer::computeLengthOfTheEdgesConnectedToVertex(int vertex ){
	...
	for(int i=0; i<NO ; i++)
		...
		{
			edge_length[i][vertex] =  edge_length[vertex][i]  =  Utils::dist(positions[i], positions[vertex]);
			if(debug) cout << "edge_length["<<i<<"]["<<vertex<<"] " << edge_length[i][vertex] <<endl;
		}
}



void CartesianRealizer::locateTetrahedron( int tetrahedron_index, double posi[][3] , bool mirror){
	double position[4][3];
	...
	bool foundNan = false;
	Utils::lenToTetra(edgeLengths, position[0], position[1], position[2], position[3],  mirror);
	for(int i=0; i<4; i++)
	{
		posi[i][0] = position[i][0];  posi[i][1] = position[i][1];  posi[i][2] = position[i][2];
		if(std::isnan(posi[i][0]) || std::isnan(posi[i][1]) || std::isnan(posi[i][2]) )
			foundNan = true;
	}
	...
	for(int i=0; i<4; i++)
		for(int j=i+1; j<4; j++)
			if( abs(edge_length[inds[i]][inds[j]]-Utils::dist(posi[i],posi[j])) > .6){
				disturbed = true;
			}
	...
	if( (vc < 15  && vc >= 0 && disturbed)  ||  foundNan )
	{
		...
		posi[1][0] = edge_length[ inds[0] ][ inds[1]]; 	posi[1][1] = 0; 	posi[1][2] = 0;

		double ac= edge_length[ inds[0] ][ inds[2] ], ab= edge_length[ inds[0] ][ inds[1] ], bc=edge_length[ inds[1] ][ inds[2] ];
		double cosCAB =  (ac*ac + ab*ab - bc*bc)/ (2*ac*ab) ;  double sinCAB = sqrt( abs(1-cosCAB*cosCAB) );
		posi[2][0] = edge_length[ inds[0] ][ inds[2] ]*cosCAB; 	posi[2][1] = edge_length[ inds[0] ][ inds[2] ]*sinCAB; 	posi[2][2] = 0;

		ac= edge_length[ inds[0] ][ inds[3] ]; ab= edge_length[ inds[0] ][ inds[1] ]; bc=edge_length[ inds[1] ][ inds[3] ];
		cosCAB =  (ac*ac + ab*ab - bc*bc)/ (2*ac*ab) ;  sinCAB = sqrt( abs(1-cosCAB*cosCAB) );
		posi[3][0] = edge_length[ inds[0] ][ inds[3] ]*cosCAB; 	posi[3][1] = edge_length[ inds[0] ][ inds[3] ]*sinCAB; 	posi[3][2] = 0;

		double nposi[2]; nposi[0] = posi[3][0]; nposi[1] = -1 * posi[3][1];
		double cd1 = sqrt( (posi[2][0]-posi[3][0])*(posi[2][0]-posi[3][0]) + (posi[2][1]-posi[3][1])*(posi[2][1]-posi[3][1])  );
		double cd2 = sqrt( (posi[2][0]-nposi[0])*(posi[2][0]-nposi[0]) + (posi[2][1]-nposi[1])*(posi[2][1]-nposi[1])  );
		double cd = 	edge_length[ inds[2] ][ inds[3] ];

		if(  abs(cd-cd1) > abs(cd-cd2) )
			posi[3][0] = nposi[0];  posi[3][1] = nposi[1];
...
bool CartesianRealizer::setBaseTetra( bool mirror){
	...
	if( inds[0] < NO/2 && inds[1] <NO/2 && inds[2]<NO/2 && inds[3] <NO/2  ) //inside of same helix
	{
		for(int i=0; i<4; i++ ){
			for(int j=0; j<3; j++)
				positions[ inds[i] ][j] = helA->getAtomAt( verticesA[ inds[i] ] )->getLocation()[j];

		}
		return true;
	}
	else if( inds[0] >= NO/2 && inds[1] >= NO/2 && inds[2] >= NO/2 && inds[3] >= NO/2  ) //inside of same helix
	{
		for(int i=0; i<4; i++ ){
			for(int j=0; j<3; j++)
				positions[ inds[i] ][j] = helB->getAtomAt( verticesB[ inds[i]-NO/2 ]  )->getLocation()[j];

		}
		return true;
	}

	if(debug) cout<<  "base " << inds[0] << " " << inds[1] << " " << inds[2]<< " " << inds[3] << endl;
...
//	eaB_[1] = acos(eaB_[1]) * 180 / PI;
	...
	//eaB_[1] = abs( eaB_[1] ); //FOR TEST REASON SINCE There are, however, other sets of Euler angles which give the same rotation matrix
//	int sign = Utils::sign(cos_eaB1);  //R(a,b,c) = R(-a, b+PI, -c) = R( -a, b-PI, -c) and since  cos(b+PI)=cos(b-PI)= -cos(b) so when I invert the sign of cos(eab1), we need to also invert the sign of eab0 and eab2
	//hence if you take abs value of eab1, you should invert eab0, eab2 as well.
...
//	}
	//also i am assuming eaB_= RB.eulerAngles(2, 0, 2); always gives eab0 and eab2 in between [-180, 180] by observation
...
void CartesianRealizer::printPositionsAndConnections( vector<int> ind){
	...
	if(debug) cout<<  "indices " << ind[0] << " " << ind[1] << " " << ind[2]<< " " << ind[3] << endl;
