...
PredefinedInteractions::PredefinedInteractions(vector<vector<string> > values, size_t labelAcol, size_t labelBcol,size_t radiuscol){
		this->dist2Defined = false;
		this->distTableDefined = false;
		cout << "distfinder" << endl;
	for(vector<vector<string> >::iterator rowIter = values.begin(); rowIter != values.end(); rowIter++){
...
PredefinedInteractions::PredefinedInteractions(vector<vector<string> > values, size_t labelAcol, size_t labelBcol,size_t radiusmincol, size_t radiusmaxcol){
	this->dist2Defined = false;
	this->distTableDefined = false;
	for(vector<vector<string> >::iterator rowIter = values.begin(); rowIter != values.end(); rowIter++){
		string labelA = (*rowIter)[labelAcol];
...
double PredefinedInteractions::collisionLower(double distance){
	return Settings::Constraint::collisionLambda * distance + Settings::Constraint::collisionDelta;
}

double PredefinedInteractions::bondLower(double distance){
	return Settings::Constraint::bondingLowerLambda * distance + Settings::Constraint::bondingLowerDelta;
}

double PredefinedInteractions::bondUpper(double distance){
	return Settings::Constraint::bondingUpperLambda * distance + Settings::Constraint::bondingUpperDelta;
}
...
void PredefinedInteractions::assign(PredefinedInteractions *other){
	this->dist1.clear();
	for(map<pair<string,string>,double>::iterator iter = other->dist1.begin();iter != other->dist1.end();iter++){
		this->dist1[iter->first] = iter->second;
	}
	this->distTableDefined = this->dist1.size() > 0;

	this->dist2.clear();
	for(map<pair<string,string>,double>::iterator iter = other->dist2.begin();iter != other->dist2.end();iter++){
...
bool PredefinedInteractions::find(map<pair<string,string>,double > maap ,pair<string,string> val){
