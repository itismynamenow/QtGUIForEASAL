...
static int complete3trees[11][6][2] = {
		{{0,0},{0,1},{0,2},{1,1},{1,2},{2,2}}, {{0,0},{0,1},{0,2},{1,1},{1,2},{2,0}}, {{0,0},{0,1},{1,0},{1,1},{2,1},{2,2}}, // 3*3
		{{0,0},{0,1},{0,2},{1,1},{1,2},{2,3}}, {{0,0},{0,1},{0,2},{1,2},{1,3},{2,3}}, {{0,0},{0,1},{0,2},{1,2},{1,3},{2,2}}, {{0,0},{0,1},{1,1},{1,2},{1,3},{2,3}},  // 3*4
		{{0,0},{0,1},{0,2},{1,2},{1,3},{2,4}}, {{0,0},{0,1},{0,2},{1,2},{2,3},{2,4}}, {{0,0},{0,1},{0,2},{1,3},{1,4},{2,4}}, // 3*5
		{{0,0},{0,1},{0,2},{1,3},{1,4},{2,5}} // 3*6
};


CayleyParameterization::CayleyParameterization(ActiveConstraintGraph* cgk, bool basic)
{
	...
	{
		if(Settings::AtlasBuilding::parameterMinDeviation && this->contacts.size()==1){
		...
		{
			//add contacts
			for(vector<pair<int,int> >::iterator iter = this->contacts.begin(); iter != this->contacts.end(); iter++)
...
vector<pair<int,int> > CayleyParameterization::defineParameters()
{
	...
	if( asize==3 && bsize==3){  begin=0; end=2; }
	else if( (asize==4 && bsize==3) || (asize==3 && bsize==4) ){  begin=3; end=6; if(asize==4 && bsize==3) use_symmetric=true; }
	else if( (asize==5 && bsize==3) || (asize==3 && bsize==5) ){  begin=7; end=9; if(asize==5 && bsize==3) use_symmetric=true; }
	else if( (asize==6 && bsize==3) || (asize==3 && bsize==6) ){  begin=10; end=10; if(asize==6 && bsize==3) use_symmetric=true; }
	...
	for(int i=begin; i<=end; i++)  //check corresponding graph according to number of contact atoms
	{
		...
		int graph[6][2];
		for(int j=0; j<6; j++) {
			if(use_symmetric)  {graph[j][0] = complete3trees[i][j][1];  graph[j][1] = complete3trees[i][j][0]; }  //contacts is symmetric of the graph given in the list complete3trees
			else      {graph[j][0] = complete3trees[i][j][0];  graph[j][1] = complete3trees[i][j][1]; }
		}
			...
			do {  // permutates indicesB
				...
				for(vector<pair<int,int> >::iterator iter = this->contacts.begin(); iter != this->contacts.end(); iter++)
				{
					...
					for(size_t j=0; j<6; j++)
						if( indicesA[ graph[j][0] ]==(*iter).first &&  indicesB[ graph[j][1] ]+ NO/2 == (*iter).second ){
		...
		{
			for(int p=0; p<6; p++)
				temp_parameters.push_back(make_pair(indicesA[ graph[p][0] ], indicesB[ graph[p][1] ]+NO/2) ); //this also includes contacts too, but in the next step they will be eliminated.
...
// If the contacts are not part of any pre-determined graphs i.e. it is not partial 3-tree, then assign random parameters
	...
	int all_possible_parameters[7][2] = { {2,1}, {1,2}, {1,0}, {0,1}, {2,2}, {2,0}, {0,2} }; //i put 21 12 before 10, 01 to make sure the third atom is used
	if(!matched)
		for(int p=0; p<7; p++)
			temp_parameters.push_back(make_pair(all_possible_parameters[p][0], all_possible_parameters[p][1]+NO/2) );
	...
	for(vector<pair<int,int> >::iterator iter = this->contacts.begin(); iter != this->contacts.end(); iter++)
	{
		...
		{
			if( temp_parameters[j].first==(*iter).first &&  temp_parameters[j].second ==(*iter).second )
			{
	...
	if( temp_parameters.size()>no_params )
		temp_parameters.erase (temp_parameters.begin()+no_params, temp_parameters.end());
...
vector<pair<int,int> > CayleyParameterization::parameterMinDeviation() //works for 5d
{
	...
	this->verticesA.assign( currentGraph->get_unordered_setA().begin(),  currentGraph->get_unordered_setA().end() );
	this->verticesB.assign( currentGraph->get_unordered_setB().begin(),  currentGraph->get_unordered_setB().end() );
	  ...
	  for(int i2=i1+1; i2<sizeA; i2++)
	  {
	    ...
	    for(int j1=0; j1<sizeB; j1++)
	    {
	      ...
	      for(int j2=j1+1; j2<sizeB; j2++)
	      {
			...
			verticesA[1] = i1;  verticesA[2] = i2;
			verticesB[1] = j1;  verticesB[2] = j2;
...
//------------------------------
			...
			for(size_t v1 = 0; v1<NO/2; v1++){
				 ...
				 {
					connections[v1][v2] = Utils::dist(helA->getAtomAt(verticesA[v1])->getLocation(),helA->getAtomAt(verticesA[v2])->getLocation() );//The location retrieved from an atom is the actual pointer don't delete it
			...
			//set fixed edge lengths inside helix b
			 for(size_t v1 = NO/2; v1<NO; v1++){
				 ...
				 {
					connections[v1][v2] = Utils::dist(helB->getAtomAt(verticesB[v1-NO/2])->getLocation(),helB->getAtomAt(verticesB[v2-NO/2])->getLocation() );//The location retrieved from an atom is the actual pointer don't delete it
					connections[v2][v1] = connections[v1][v2];
					maxconnections[v1][v2] = connections[v1][v2];
					maxconnections[v2][v1] = connections[v1][v2];
					minconnections[v1][v2] = connections[v1][v2];
				...
				for(int v1=0; v1<NO/2; v1++)
					for(int v2=NO/2; v2<NO; v2++)
					{
						if( connections[v1][v2] == -1 ){ //if not contact, then find the max distance for edge v1-v2
							double max = 100;
							for(int i=0; i<NO; i++)
								if( maxconnections[v1][i] != -1 && maxconnections[i][v2] != -1  &&  max > (maxconnections[v1][i] + maxconnections[i][v2])  )
...
//------------------------------
			...
			while(change)  //updates min distances
			{
				...
				for(int v1=0; v1<NO/2; v1++)
					for(int v2=NO/2; v2<NO; v2++)
					{
						if( connections[v1][v2] == -1 || std::isnan(connections[v1][v2])   ){ // not a contact

							double min = 0;
							for(int i=0; i<NO; i++){
								double localmin = 100;

								if(  minconnections[v1][i] > maxconnections[i][v2]  && minconnections[v1][i]!= -1 && maxconnections[i][v2]!= -1  )
									localmin = minconnections[v1][i] - maxconnections[i][v2] ;
								else if(  minconnections[v2][i] > maxconnections[i][v1]  && minconnections[v2][i]!= -1 && maxconnections[i][v1]!= -1  )
							...
							double sum = Settings::Constraint::collisionLambda * sumradii + Settings::Constraint::collisionDelta; //todo use distancefinder collisionLowerBound method
...
//------------------------------
			...
			for(int v1=0; v1<NO/2; v1++)
				for(int v2=NO/2; v2<NO; v2++){
					edges[v1][v2-NO/2] = maxconnections[v1][v2] - minconnections[v1][v2] ;
				}


			double topdown[6][3]; //6 edges of (value, v1, v2).  (v1, v2 is indices of vertices in edges) and ( value is edge[v1][v2] )
			topdown[0][0]=100;topdown[1][0]=100;topdown[2][0]=100;  //3 mins
			topdown[3][0]=0;  topdown[4][0]=0;  topdown[5][0]=0;    //3 maxs
			...
			for(int v1=0; v1<NO/2; v1++)
				for(int v2=0; v2<NO/2; v2++)
				{
					...
					if(edges[v1][v2] < topdown[0][0])
					{
						...
						topdown[2][1] = topdown[1][1]; topdown[2][2] = topdown[1][2];
						topdown[1][1] = topdown[0][1]; topdown[1][2] = topdown[0][2];
						topdown[0][1] = v1; 		   topdown[0][2] = v2;
					}
					else if(edges[v1][v2] < topdown[1][0])
					{
						...
						topdown[2][1] = topdown[1][1]; topdown[2][2] = topdown[1][2];
						topdown[1][1] = v1; topdown[1][2] = v2;
					}
					else if(edges[v1][v2] < topdown[2][0])
					{
						topdown[2][0] = edges[v1][v2];
						topdown[2][1] = v1; topdown[2][2] = v2;
					}
					...
					if(edges[v1][v2] > topdown[5][0])
					{
						...
						topdown[3][1] = topdown[4][1]; topdown[3][2] = topdown[4][2];
						topdown[4][1] = topdown[5][1]; topdown[4][2] = topdown[5][2];
						topdown[5][1] = v1; topdown[5][2] = v2;
					}
					else if(edges[v1][v2] > topdown[4][0])
					{
						...
						topdown[3][1] = topdown[4][1]; topdown[3][2] = topdown[4][2];
						topdown[4][1] = v1; topdown[4][2] = v2;
					}
					else if(edges[v1][v2] > topdown[3][0])
					{
						topdown[3][0] = edges[v1][v2];
						topdown[3][1] = v1; topdown[3][2] = v2;
					}
...
//------------------------------
		//3 edges to be removed from top or down. compute the deviations if those 3 edges were removed from the set.
			...
			double average1 = ( sum - (topdown[0][0] + topdown[1][0] + topdown[2][0]) ) / edgeno;
			double avgsquares1 = (squares - (topdown[0][0]*topdown[0][0] + topdown[1][0]*topdown[1][0] + topdown[2][0]*topdown[2][0]) ) / edgeno;
			double deviation1 = sqrt( avgsquares1 - average1*average1);

			//case : remove top 3
			double average2 = ( sum - (topdown[3][0] + topdown[4][0] + topdown[5][0]) ) / edgeno;
			double avgsquares2 = (squares - (topdown[3][0]*topdown[3][0] + topdown[4][0]*topdown[4][0] + topdown[5][0]*topdown[5][0]) ) / edgeno;
			double deviation2 = sqrt( avgsquares2 - average2*average2);

			//case 3: remove down 1 top 2
			double average3 = ( sum - (topdown[0][0] + topdown[4][0] + topdown[5][0]) ) / edgeno;
			double avgsquares3 = (squares - (topdown[0][0]*topdown[0][0] + topdown[4][0]*topdown[4][0] + topdown[5][0]*topdown[5][0]) ) / edgeno;
			double deviation3 = sqrt( avgsquares3 - average3*average3);

			//case 4: remove down 2 top 1
			double average4 = ( sum - (topdown[0][0] + topdown[1][0] + topdown[5][0]) ) / edgeno;
			double avgsquares4 = (squares - (topdown[0][0]*topdown[0][0] + topdown[1][0]*topdown[1][0] + topdown[5][0]*topdown[5][0]) ) / edgeno;
...
//------------------------------
			...
			if( mindeviation > deviation1 && !( topdown[0][1]!=topdown[1][1] && topdown[1][1]!=topdown[2][1] && topdown[0][1]!=topdown[2][1] && topdown[0][2]!=topdown[1][2] && topdown[1][2]!=topdown[2][2] && topdown[0][2]!=topdown[2][2]) )
				if(!( (topdown[0][1]==topdown[1][1] && topdown[1][1]==topdown[2][1])  || (topdown[0][2]==topdown[1][2] && topdown[1][2]==topdown[2][2]) ) )
			{
				mindeviation = deviation1;
				std::stringstream removeset1;
				removeset1 << 0 << topdown[0][1] << 0 << topdown[0][2]+N << " " << 0 << topdown[1][1] << 0 << topdown[1][2]+N << " " << 0 << topdown[2][1] << 0 << topdown[2][2]+N << " ";
				removeset = removeset1.str();
			}
			if( mindeviation > deviation2 && !( topdown[3][1]!=topdown[4][1] && topdown[4][1]!=topdown[5][1] && topdown[3][1]!=topdown[5][1] && topdown[3][2]!=topdown[4][2] && topdown[4][2]!=topdown[5][2] && topdown[3][2]!=topdown[5][2]) )
				if(!( (topdown[3][1]==topdown[4][1] && topdown[4][1]==topdown[5][1])  || (topdown[3][2]==topdown[4][2] && topdown[4][2]==topdown[5][2]) ) )
			{
				mindeviation = deviation2;
				std::stringstream removeset2;
				removeset2 << 0 << topdown[3][1] << 0 << topdown[3][2]+N << " " << 0 << topdown[4][1] << 0 << topdown[4][2]+N << " " << 0 << topdown[5][1] << 0 << topdown[5][2]+N << " ";
				removeset = removeset2.str();
			}
			if( mindeviation > deviation3 && !( topdown[0][1]!=topdown[4][1] && topdown[4][1]!=topdown[5][1] && topdown[0][1]!=topdown[5][1] && topdown[0][2]!=topdown[4][2] && topdown[4][2]!=topdown[5][2] && topdown[0][2]!=topdown[5][2]) )
				if(!( (topdown[0][1]==topdown[4][1] && topdown[4][1]==topdown[5][1])  || (topdown[0][2]==topdown[4][2] && topdown[4][2]==topdown[5][2]) ) )
			{
				mindeviation = deviation3;
				std::stringstream removeset3;
				removeset3 << 0 << topdown[0][1] << 0 << topdown[0][2]+N << " " << 0 << topdown[4][1] << 0 << topdown[4][2]+N << " " << 0 << topdown[5][1] << 0 << topdown[5][2]+N << " ";
				removeset = removeset3.str();
			}
			if( mindeviation > deviation4 && !( topdown[0][1]!=topdown[1][1] && topdown[1][1]!=topdown[5][1] && topdown[0][1]!=topdown[5][1] && topdown[0][2]!=topdown[1][2] && topdown[1][2]!=topdown[5][2] && topdown[0][2]!=topdown[5][2]) )
				if(!( (topdown[0][1]==topdown[1][1] && topdown[1][1]==topdown[5][1])  || (topdown[0][2]==topdown[1][2] && topdown[1][2]==topdown[5][2]) ) )
			{
				mindeviation = deviation4;
				std::stringstream removeset4;
				removeset4 << 0 << topdown[0][1] << 0 << topdown[0][2]+N << " " << 0 << topdown[1][1] << 0 << topdown[1][2]+N << " " << 0 << topdown[5][1] << 0 << topdown[5][2]+N << " ";
...
//------------------------------
			...
			if(overallmindeviation > mindeviation){
				...
				for(int t = 0; t<paramset.size();  t=t+5){
					...
					if( find(this->verticesA.begin(), this->verticesA.end(), verticesA[v1] ) == this->verticesA.end() )
						this->verticesA.push_back( verticesA[v1] );
					if( find(this->verticesB.begin(), this->verticesB.end(), verticesB[v2] ) == this->verticesB.end() )
				...
				for(int t = paramset.size()-5; t>=0; t=t-5){
					...
					if((maxconnections[v1][v2] - minconnections[v2][v1]) != 0 )
						volume = volume * (maxconnections[v1][v2] -	minconnections[v2][v1]) ;
		    		...
		    		for(int k=j+1; k<NO ; k++)
		    			if(  vertex_position[k] ) //these points located before.
							for(int l=0; l<NO; l++) //find a point with unknown location and then set it
								if( !vertex_position[l]  && edge[i][l] && edge[j][l] && edge[k][l]  ){
									inds.push_back(l); inds.push_back(i); inds.push_back(j); inds.push_back(k);
...
void CayleyParameterization::identifyFixedEdgesConnectedToVertex(int new_located_vertex){

	for(int i=0; i<NO ; i++)
		if( vertex_position[i]  &&  !edge[i][new_located_vertex] ){
			edge[i][new_located_vertex] = edge[new_located_vertex][i] = true;
			fixedEdge[i][new_located_vertex] = fixedEdge[new_located_vertex][i] = true;
		}
}
					...
					for(int k=j+1; k<NO/2; k++)
						if(  edge[i][k] && edge[j][k])
							for(int l=k+1; l<NO/2; l++)
								...
								{
									inds.push_back(j); inds.push_back(k); inds.push_back(l); inds.push_back(i);
									tetrahedra.push_back(inds);
									vertex_position[j] = vertex_position[k]=vertex_position[l]=vertex_position[i]=true;
									return true; //done
								}
...
void CayleyParameterization::printTetras()
{
	...
	for( int i=0; i<this->tetrahedra.size(); i++)
		cout << " " << this->tetrahedra[i][0] << " " << this->tetrahedra[i][1] << " " << this->tetrahedra[i][2] << " " << this->tetrahedra[i][3] << endl;
						...
						if(first)
						{
							...
							for(int k =0; k<this->parameters.size(); k++){
								if( tetraHedraParams[j][k] ){
									int before = Utils::findPair(oparameters, this->parameters[k]);
									if(before < smallest)
										smallest = before;
								}
							}
