...
private:    //private copy-ctor and assignment operator ensure the lock never gets copied, which might cause issues.
    Spinlock operator=(const Spinlock & sl);
	...
	public:
	    ...
	    bool try_lock() {
	        int ret = pthread_spin_trylock(&m_lock); //Upon successful completion, these functions shall return zero; otherwise, an error number shall be returned to indicate the error.
	        return ret;
...
public:
	 ...
	 */
	#ifdef SERVER
	SaveLoader(string relativePath, MolecularUnit *a, MolecularUnit *b, mongocxx::client *mongoDBClient);
	#else
	 ...
	 */
	void writeOrientationInPDBformat(MolecularUnit *hA,MolecularUnit *hB,Orientation *orien, string filename);
	 ...
	 * @see readOrientation
	 */
	void loadActiveConstraintRegion(istream &file,   ActiveConstraintRegion *output);
	...
	void writeCayleyPointtoJSON(CayleyPoint* pnt, string file, int nodeID, bool witness = false);
...
private:
	...
	/**
	 * Lockers to control data i/o
	 */
	pthread_mutex_t inUse;
	pthread_mutex_t atlasInUse;
	// pthread_spinlock_t mapViewInUse;
