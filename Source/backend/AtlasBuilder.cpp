...
AtlasBuilder::AtlasBuilder(MolecularUnit* helA, MolecularUnit* helB, SaveLoader* snl, PredefinedInteractions *df, Atlas* atlas)
:verbose(false)
{
	this->snl = snl;
	this->atlas = atlas;
...
void AtlasBuilder::setData(MolecularUnit* helA, MolecularUnit* helB, SaveLoader* snl, PredefinedInteractions *df, Atlas* atlas)
{
    {
        this->snl = snl;
        this->atlas = atlas;
...
void AtlasBuilder::create_initial_contactGraphs_for_virusCase()
{
	...
	for (PredefinedInteractions::dist_iterator dit1 = this->df->dist2begin(); 	dit1 != this->df->dist2end(); dit1++)
	{
		for (PredefinedInteractions::dist_iterator dit2 = dit1; dit2 != this->df->dist2end(); dit2++)
		{
			if (dit1 == dit2)
				continue;
			...
			if (da < Settings::RootNodeCreation::initial4DContactSeparation_low || da > Settings::RootNodeCreation::initial4DContactSeparation_high || db < Settings::RootNodeCreation::initial4DContactSeparation_low || db > Settings::RootNodeCreation::initial4DContactSeparation_high)
				continue;
...
//				if ((da + ar1 + ar2 >= db - br1 - br2 - Settings::Collision::threshold*2) &&
//				    (da - ar1 - ar2 <= db + br1 + br2 + Settings::Collision::threshold*2) ){
			...
			// create the contact graph of pair of atom and add it into the rootGraph list
			{
				...
				ActiveConstraintGraph* initial = new ActiveConstraintGraph(parts,	this->a, this->b);
...
{
	if(this->verbose)cout << "create_initial_4d_contactGraphs_usingDumbbells" <<endl;
	...
	if(this->verbose)cout << "got dumbbells setA["<<vectorA.size()<<"] setB["<< vectorB.size()<<"]" << endl;
	...
	for(iter = vectorA.begin();iter != vectorA.end();iter++)
		for(it = vectorB.begin();it != vectorB.end();it++){
			...
			double z_slide_first  = abs( (a->getAtomAt((*iter).first))->getLocation()[2] - (b->getAtomAt((*it).first))->getLocation()[2]  ) ;   //i can do this because they are originally aligned to same position in the z axis
			double z_slide_second = abs( (a->getAtomAt((*iter).second))->getLocation()[2] - (b->getAtomAt((*it).second))->getLocation()[2] ) ;   //i can do this because they are originally aligned to same position in the z axis


			double da = Utils::dist((a->getAtomAt((*iter).first))->getLocation(),	(a->getAtomAt((*iter).second))->getLocation() );
		...
		//	double ar12 = a->getAtomAt( (*iter).first )->getMinDist( a->getAtomAt( (*iter).second) );  //if( ar12 == -1 )  ar12 = ar1 + ar2;
			...
			double db = Utils::dist((b->getAtomAt((*it).first))->getLocation(),		(b->getAtomAt((*it).second))->getLocation() );
		...
		//	double br12 = b->getAtomAt( (*it).first )->getMinDist( b->getAtomAt( (*it).second) );
			...
			if(  (da + ar12 >= db - br12) && ( da - ar12 <= db + br12)  )
			{
				...
				if( !Settings::RootNodeCreation::useParticipatingAtomZDistance || (z_slide_first<Settings::RootNodeCreation::ParticipatingAtomZDistance && z_slide_second<Settings::RootNodeCreation::ParticipatingAtomZDistance) ) //( abs((*iter).first - (*it).first)<4  &&   abs((*iter).second - (*it).second)<4  )  //to choose dumbbells proportional to the place in the helix (the other direction is ignored now)
				{
					//first paring
				...
				{
					double z_slide_first_reverse  = abs( (a->getAtomAt((*iter).first))->getLocation()[2] - (b->getAtomAt((*it).second))->getLocation()[2]  ) ;   //i can do this because they are originally aligned to same position in the z axis
					double z_slide_second_reverse = abs( (a->getAtomAt((*iter).second))->getLocation()[2] - (b->getAtomAt((*it).first))->getLocation()[2] ) ;   //i can do this because they are originally aligned to same position in the z axis

					if(!Settings::RootNodeCreation::useParticipatingAtomZDistance ||  ( z_slide_first_reverse<Settings::RootNodeCreation::ParticipatingAtomZDistance  &&   z_slide_second_reverse<Settings::RootNodeCreation::ParticipatingAtomZDistance) ) //Settings::RootNodeCreation::closeByDumbbellsAmount=5
					{
						vector<pair<int,int> > parts;
...
void AtlasBuilder::create_initial_5d_contactGraphs()
{
	...
	for(size_t i=0; i<helA.size(); i++)
	{
	   for(size_t j=0; j<helB.size(); j++)
	   {
			...
			double z_slide  = abs( helA[i]->getLocation()[2] - helB[j]->getLocation()[2]  ) ;   //i can do this because they are originally aligned to same position in the z axis


			if( !Settings::RootNodeCreation::useParticipatingAtomZDistance || z_slide < Settings::RootNodeCreation::ParticipatingAtomZDistance) //( abs(i- j)<4 )  //to choose dumbbells proportional to the place in the helix (the other direction is ignored now)
			{
				vector<pair<int,int> > parts; //holds the indices of atoms
...
//--------------------------------
	...
	if(this->verbose)cout << "Created "<< this->rootGraphs.size() << " contactIDs" << endl;
	for(list< pair<ActiveConstraintGraph*, bool> >::iterator iter = this->rootGraphs.begin();  iter != this->rootGraphs.end();	iter++)
	{
		if(this->verbose){
...
//--------------------------------
	...
	list< pair<ActiveConstraintGraph*, bool> >::iterator iterl = this->rootGraphs.begin();
...
{
//		pthread_spin_lock(&rootGraphsInUse);
		empty = true;
		ActiveConstraintGraph* nextd;
		for(list< pair<ActiveConstraintGraph*, bool> >::iterator it=this->rootGraphs.begin(); it!=this->rootGraphs.end(); it++)
		{
...
//		pthread_spin_unlock(&rootGraphsInUse);
		return nextd;
}

void AtlasBuilder::refigureFlip(Orientation* orr, std::vector<std::vector<int> > flipScheme, SaveLoader* snlr){
    ...
    std::vector<std::vector<int> > tetras = flipScheme;
                    ...
                    vector<double> trans_mat = Utils::getTransMatrix(fromBOrr,
                            toBOrr);
                    ...
                    for (int x = 0; x < tetras.size(); x++) {
                        ...
                        for (int y = 0; y < 4; y++) {
                            ...
                            if (tetras[x][y] > 5) {
                                tetraCoordinates.push_back(
                                        snlr->b->getTransformedCoordinates(
                                                (tetras[x][y] - 6), trans_mat));
                            } else {
                                tetraCoordinates.push_back(
                                        snlr->a->getCoordinates(tetras[x][y]));
                            }
                        }
                        ...
                        /*
                        Vector3 testVector_1 = new Vector3(vec_12);
                        Vector3 testVector_2 = new Vector3(vec_13);
                        Vector3 testVector_3 = new Vector3(vec_14);

                        double l1 = testVector_1.squared_length();
                        double l2 = testVector_2.squared_length();
                        double l3 = testVector_3.squared_length();
                    ...
                    orr->setFlipNum(flip);
                    //cout <<"New flip: " << flip << " should match: " << orr->getFlipNum() << endl;
...
void AtlasBuilder::startAtlasBuilding(){
	...
	num_samples = 0;
	std::ofstream ofs;
	ofs.open ("Samples.txt", std::ofstream::out | std::ofstream::app);
	
	cout << "AtlasBuilder::startAtlasBuilding: this->rootGraphs.size() = " << this->rootGraphs.size() << endl;
	...
	if(!Settings::AtlasBuilding::stop ) {
		...
		if( success == 1)
		{
			cout << "AtlasBuilder::startAtlasBuilding: We start sampling rootGraph " << currentrootGraph << " out of " << this->rootGraphs.size() << " (node number " << nodenum << ")" << endl;
			...
			if(symmNodeNum != -1) // if there is a node with symmetric contact graph, then we want to exploit its parameters
			{
				...
				ActiveConstraintGraph* reverseNode = new ActiveConstraintGraph(sparticp, this->a,this->b, splines);
		...
		ofs <<"The total number of samples for node " <<nodenum<< " is "<< num_samples<< endl;
		num_samples = 0;
	...
	cout << "FINISHED AtlasBuilder::startAtlasBuilding: this->rootGraphs.size() = " << this->rootGraphs.size() << endl;
	cout << "Finding number of paths of length "<<Settings::Paths::pathLength<<endl;
	ofs << "The sampling was completed in "<<elapsed_secs<<" seconds"<<endl;
	ofs.close();
...
void AtlasBuilder::determineStepSizeDynamically(ActiveConstraintGraph *cgK, ActiveConstraintRegion * region, bool dense, CayleyParameterization * cparam )
{
	...
	*/
	double contact_lengthUpper = df->bondingUpperBound( chart->getAtom(0), chart->getAtom(6) );
	double contact_lengthLower = df->bondingLowerBound( chart->getAtom(0), chart->getAtom(6) );
	...
	if( chart->initializeChart(false, region) &&  cparam->is_partial3tree()   )
		 for( ;   !chart->doneGrid()&& !Settings::AtlasBuilding::stop;		chart->stepGrid()  )
			 volume++;
	 ...
	 * nspd = pow(tns, 0.2)
	 * nspd1*s1=nspd2*s2
	 * s2 = s1* nspd1/nspd2 = s1* pow(tns1, 0.2)/pow(tns2, 0.2) = s1* pow(tns1/tns2, 0.2) = s1 / pow(tns2/tns1, 0.2)
	 */
...
void AtlasBuilder::createChildContactGraps_fromTheBoundary(
        ...
        bool& noGoodOrientation, int& noPoints, CayleyPoint* EntryCayleyPoint)
{
	...
	for (int c = 0; c < contactList.size(); c++)  //TRY ALL CONTACT LIST
        ...
        //Entry point pushback
        wit_orr_toSend->pushbackEntryPoint(rnode->getID(), EntryCayleyPoint ,orie_on_boundary);
		...
		createChildNodes(child_node, contactList, wit_orr_toSend,
				this->atlas, this->snl, firstPath);
...
void AtlasBuilder::findBoundary(list<Orientation*>::iterator ori_on_lattice, ConvexChart* desc,
		ActiveConstraintGraph* cgK, AtlasNode* rnode, ActiveConstraintRegion *region,
		ConstraintCheck* detector, bool bret, bool& noGoodOrientation,
        int& noPoints, bool& boundary_ori_found_and_saved, CayleyPoint* EntryCayleyPoint)
{
	...
	while (!desc->stepAround() && !Settings::AtlasBuilding::stop)
	{
		...
		Orientation* orie_on_boundary =
                CartesianRealizer::computeRealization(cgK, desc, flip, fail, flipScheme); //if needs maple, then it will always return first root not the one with specified flip !
		...
		if (!fail &&  detector->stericsViolated(orie_on_boundary) ) {
			...
			num_samples++;
			orie_on_boundary = CartesianRealizer::computeRealization(cgK, desc,
			...
			if ( binvalid
				...
				createChildContactGraps_fromTheBoundary(contactList2, ori_on_lattice,
						cgK, bret, orie_on_boundary, rnode,
                        noGoodOrientation, noPoints, EntryCayleyPoint);
                if (Settings::Saving::saveBoundary) //(since we do not save it to child)   //o not add witness point to parent node!
				{
...
/*
 * This is a copy of the initial AtlasBuilder, though the actual AtlasBuilder may have changed a bit.
 *
...
bool AtlasBuilder::sampleTheNode(AtlasNode *rnode, bool dense, Orientation* coming_witness_ori, bool continu, bool bret )
{
	int from = rnode->getID();  // from is the node number of the incoming node as stored in the Roadmap object.
	if (this->verbose) cout<< "AtlasBuilder::MySample: Started node " << from << endl;
	...
	if( continu )  noGoodOrientation = !rnode->hasAnyGoodOrientation();

    //TODO: start added code - BRIAN
        ...
        if (rnode->getFlipScheme().empty()) {
            ...
            if (chart->partial3tree) {
                ...
                //choose the parent w/ the lowest node ID, as that will be the first parent (atlas is built w/dfs)
                //is it the case here that rnode always only has one parent?
	...
	if( coming_witness_ori != NULL){
        CayleyPoint* wp4 = new CayleyPoint(coming_witness_ori, cgK->getParamLines(), a, b, region->getPointsCreated() );   //deletion of coming_witness_ori is handled WHERE IT IS CREATED.

        refigureFlip((wp4->getOrientations()[0]), rnode->getFlipScheme(), snl);
        region->incrementPointsCreated();
	...
	addReverseWitness(rnode, cgK, chart, region, coming_witness_ori, noGoodOrientation);
    ...
    if(  desc->is_partial3tree()  && chart->initializeChart(continu, region) )
    {
		...
		for(;
			!chart->doneGrid() && !Settings::AtlasBuilding::stop;
			chart->stepGrid())
		{
			// find all realization for the grid point
            real = findRealizations(cgK, chart, rnode->getFlipScheme());
			vector<double> out = chart->getPoint();
			CayleyPoint* p4d = new CayleyPoint(out);
            p4d->setID(region->getPointsCreated());
            region->incrementPointsCreated();
            p4d->setRealizable( !real.empty() );

            if( !real.empty() )
            for(list<Orientation*>::iterator ori_on_lattice = real.begin(); ori_on_lattice != real.end()&& !Settings::AtlasBuilding::stop; ori_on_lattice++)
			{
				...
				if( !detector->stericsViolated(*ori_on_lattice) )
				{
					...
					if (Settings::RootNodeCreation::createChildren)    //set it false JUST TO CREATE INTERIOR of root nodes
					{
						...
						findBoundary(ori_on_lattice, chart, cgK, rnode, region, detector,
								bret, noGoodOrientation,
                                noPoints, boundary_ori_found_and_saved, p4d);
					}
...
//							p4d->badAngle = true;  //commented since incrementBadAngleN done above
	...
	if (this->verbose) cout<< "AtlasBuilder::MySample: Finished node " << from << endl;
...
AtlasNode* AtlasBuilder::createChildNode(AtlasNode *rnode, ActiveConstraintGraph *child_CG, Orientation* wit_orr_toSend, bool & savedOnce, bool leafWitness, bool & noGoodOrientation, int & noPoints, bool bret)
{
...
//	if(!contin)  //contin=true does not mean they are connected, maybe child_node is created by another node than 'rnode'
	...
	if(  success == 1 || (contin && !Settings::AtlasBuilding::breadthFirst ) )  //during 'breathFirst', we create child node but do not sample it. Sampling will be done after parent node is completed. Hence if contin is true, then that means child node is already created then no need to have recursive call.
	{
		...
		//MAIN RECURSIVE CALL
		if( this->sampleTheNode(child_node, false, wit_orr_toSend, contin, bret) )  //if there is a realization with good angle at the childs, then set noGoodOrientation to false
			noGoodOrientation = false;
			...
			if( !wit_orr_toSend->angleViolated() )
			{
				//if(!child_CG->isWitness(from))
				{
                    CayleyPoint* wp4 = new CayleyPoint(wit_orr_toSend,
                     child_CG->getParamLines(), a, b, child_node->getACR()->getPointsCreated()); //copy of wit_orr_toSend is added to wp4
                    //TODO: test - BRIAN_wit
                    refigureFlip(wp4->getOrientations()[0], child_node->getFlipScheme(), snl);

                    child_node->getACR()->incrementPointsCreated();
					...
					pthread_mutex_unlock(&space_sync);
		pthread_mutex_lock(&space_sync);
					wp4->trim_PointMultiD();  delete wp4;
		pthread_mutex_unlock(&space_sync);
...
*/
void AtlasBuilder::createChildNodes(AtlasNode *rnode_prime, list<pair<int,int> > contactList, Orientation* ori ,  Atlas* atlas_prime,  SaveLoader* snlr, bool firstPath){

	return;

	if( Settings::AtlasBuilding::breadthFirst || !Settings::RootNodeCreation::createChildren )  return;
	...
	for(int c=0; c<contactList.size(); c++)  //TRY ALL CONTACT LIST
	{
		...
		if( cgKchilds[c]->isDependent() ||  ( cgKchilds[c]->constraintSize() >= 6 && !cgKchilds[c]->mayHaveThirdAtom() )    )
		{
			delete cgKchilds[c];  	cgKchilds[c]=NULL;
			contactList.push_back( contact );
			continue;
		}
		...
		if(success == 0){ //exists
			...
			CayleyParameterization *desc = new CayleyParameterization( cgKchilds[c] ); //set parameters to compute witness point correctly.  // to make it run faster, calls the 2. constructor of description class which is designed to set only parameters.
        ...
        refigureFlip(orr, child_node->getFlipScheme(), snlr);
		...
		if( orr!= NULL &&  !orr->angleViolated() ){
			...
			if(Settings::Saving::saveWitnessToFinalChild && firstPath && (contactList.size() == 0 || child_node->getDim() == 0))  // TO SAVE WITNESS POINT 'ONLY' AT THE first leaf NODE!
			{
                CayleyPoint* wp4 = new CayleyPoint(orr,
                 cgKchilds[c]->getParamLines(), snlr->a, snlr->b, child_node->getACR()->getPointsCreated()); //since this->a, b is not static, i used snlr
		...
		delete orr; orr = NULL; //since findPoint creates a new orientation (which is copy of orr) then adds it to wp4, you need to delete this orr!


		createChildNodes(child_node, contactList, ori, atlas_prime, snlr, firstPath);//i assumed contactList passed by value, i mean another list is created with same contacts, so the changes to contactList in the recursive call will not affect current contactList
		firstPath = false;
		contactList.push_back( contact );
...
void AtlasBuilder::addReverseWitness(AtlasNode *rnode, ActiveConstraintGraph *cgK, ConvexChart *chart, ActiveConstraintRegion *region, Orientation* coming_witness_ori, bool & noGoodOrientation)
{
		if(Settings::General::reverseWitness){
			...
			for(int i = 0; i < edges.size(); i++){
				...
				CayleyPoint* wp4 = new CayleyPoint(ornt, cgK->getParamLines(), a, b );
				chart->setWitnessPoint(wp4);
				if( !coming_witness_ori->angleViolated() ){
					pthread_mutex_lock(&space_sync);
					region->insertWitness(wp4);
...
void AtlasBuilder::refineMap()
{
	...
	*/
	for(vector<AtlasNode*>::iterator iter = nodes.begin();	iter != nodes.end();	iter++){
		if((*iter)->getDim() == 0){
	...
	while(!cgmarker.empty()){
		pair<ActiveConstraintGraph*,vector<pair<ActiveConstraintGraph*,Orientation*> > > res = cgmarker.pop();
		...
		for(int i = 0; i < edges.size(); i++){
			...
			if(atlas->findNodeNum(res.second[i].first) == -1 ||
					!atlas->isConnected(nodenum,atlas->findNodeNum(res.second[i].first))){
...
list<Orientation*> AtlasBuilder::findRealizations(ActiveConstraintGraph *cgK, ConvexChart* des, vector<vector<int> > flipScheme ){
	...
	if( des->partial3tree )
		...
		{
			bool fail;
            Orientation *relz = CartesianRealizer::computeRealization(cgK, des, i, fail, flipScheme);
			if( !fail )
			{
