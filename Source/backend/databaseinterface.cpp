...
{
    for(list<pair<ofstream*, int> >::iterator iter = openFiles.begin(); 	iter != openFiles.end(); iter++)
    {
        if(  (*iter).second == nodenum )
            return  (*iter).first;
    }

    // if file not in buffer, open/create it
    // ofstream outFile;
    stringstream name;
    ...
    newFile.first = new ofstream( (name.str()).c_str(),ios_base::app | ios_base::out);
...
void SaveLoader::readContactGraph0(ifstream &file,   ActiveConstraintRegion *output){
        // get length of file:
...
void SaveLoader::writeCayleyPointtoJSON(CayleyPoint* pnt, std::string file, int nodeID, bool witness) {
  ...
  auto CayleyParameterValuesBSON = bsoncxx::builder::basic::array{};
  ...
  auto OrientationsArrayBSON = bsoncxx::builder::basic::array{};
  ...
  auto entryPointArrayBSON = bsoncxx::builder::basic::array{};

  vector<int> cayleyPointBoundary;

  for(int i = 0; i < pnt->dim(); i++){
    CayleyParameterValues[i] = ((*pnt)[i]);
    CayleyParameterValuesBSON.append((*pnt)[i]);
  }
  ...
  for(vector<Orientation*>::iterator iter = ornt.begin();iter != ornt.end();iter++){
    ...
    auto fbBSON = bsoncxx::builder::basic::array{};

    //An array to hold to
    Json::Value tbJSON(Json::arrayValue);
    auto tbBSON = bsoncxx::builder::basic::array{};

    //An array to hold boundary
    Json::Value boundaryJSON(Json::arrayValue);
    auto boundaryBSON = bsoncxx::builder::basic::array{};
    ...
    auto orientationBSON = bsoncxx::builder::stream::document{}; 
    
    bsoncxx::document::value orientationBSONdocument = orientationBSON
	  << "fb" << fbBSON
	  << "tb" << tbBSON
	  << "boundaries" << boundaryBSON
	  << "flip" << (int)(*iter)->getFlipNum()
	  << bsoncxx::builder::stream::finalize;



    Orientations[orients] = orientation;
    OrientationsArrayBSON.append(orientationBSONdocument);
  }
  ...
  auto cayleyPointBoundaryBSON = bsoncxx::builder::basic::array{};
  for(std::vector<int>::iterator iter = cayleyPointBoundary.begin(); iter != cayleyPointBoundary.end(); iter++) {
      ...
      cayleyPointBoundaryBSON.append(*iter);
  }
  ...
  auto cayleyPointBSON = bsoncxx::builder::stream::document{}; 

  bsoncxx::document::value cayleyPointBSONdocument = cayleyPointBSON 
	  << "NodeID" << nodeID 
	  << "Witness" << witness 
	  << "CayleyPoint" << CayleyParameterValuesBSON 
	  << "Realizable" << pnt->isRealizable() 
	  << "Z-index" << pnt->zIndex 
	  << "Orientations" << OrientationsArrayBSON 
	  << "BadAngle" << pnt->getBadAngleN() 
	  << "Collision" << pnt->getCollidN() 
	  << "Boundaries" << cayleyPointBoundaryBSON 
	  << "EntryPoints" << entryPointArrayBSON 
	  << bsoncxx::builder::stream::finalize;
