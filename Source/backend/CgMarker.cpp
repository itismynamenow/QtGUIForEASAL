...
void CgMarker::mark(AtlasNode* rnode){
	...
	if(region->getSpace().size() == 0 || !region->getSpace().at(0)->hasOrientation() ){
	...
	Orientation* orr = new Orientation(region->getSpace().at(0)->getOrientations().at(0));
	int ref_count = 0;

	for(int i = 0; i < ancestors.size(); i++){
		pair<CgMarker::iterator,bool > res =
				cgs.insert(ancestors[i]);
		...
		if(cgchildren.find(ancestors[i]) == cgchildren.end()){ // initial the children array for newly added cg
			cgchildren[ancestors[i]] = vector<pair<ActiveConstraintGraph*,Orientation*> >();
		}
	}
	...
	for(int i = 0; i < ancestors.size(); i++){
		for(int j = i+1; j < ancestors.size(); j++){	// for each ancestors
			...
			if(parent != -1 && child != -1){
				vector<pair<ActiveConstraintGraph*, Orientation*> >& children = cgchildren[ancestors[parent]];
				int count = 0;
				for(vector<Edge>::iterator eit = children.begin(); eit != children.end(); eit++){
...
pair<ActiveConstraintGraph*,vector<pair<ActiveConstraintGraph*, Orientation*> > > CgMarker::pop(){
	pair<ActiveConstraintGraph*,vector<pair<ActiveConstraintGraph*, Orientation*> > > res;
...
vector<CgMarker::Edge> CgMarker::getEdge(ActiveConstraintGraph* cg){
	...
	ActiveConstraintGraph* ncg = new ActiveConstraintGraph(parts,cg->getMolecularUnitA(),cg->getMolecularUnitB() );
