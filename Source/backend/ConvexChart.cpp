...
ConvexChart::ConvexChart(ActiveConstraintGraph* cgk, bool dense, CayleyParameterization* cayleyParam, PredefinedInteractions *df)
{
	if( Settings::Sampling::JacobianSampling ){
		for(size_t i = 0; i<this->pr1 ; i++)
			for(size_t j = 0; j<this->pr2 ; j++)
				for(size_t k = 0; k<this->pr3 ; k++)
					for(size_t l = 0; l<this->pr4 ; l++)
						for(size_t m = 0; m<this->pr5 ; m++)
							for(size_t n = 0; n<this->pr6 ; n++)
...
void ConvexChart::computeFixEdgeDistances(){
	 ...
	 for(size_t i = 0; i<this->verticesA.size(); i++){ //inside helix a
		 ...
		 {
			this->edge_length[i][j] = Utils::dist(helA->getAtomAt(verticesA[i])->getLocation(),helA->getAtomAt(verticesA[j])->getLocation() );//The location retrieved from an atom is the actual pointer don't delete it
			setMinMaxParam(i, j);
		 }
	 }
	 ...
	 for(size_t i = 0; i<this->verticesB.size(); i++){ //inside helix b
		 ...
		 {
			this->edge_length[i+NO/2][j+NO/2] = Utils::dist(helB->getAtomAt(verticesB[i])->getLocation(),helB->getAtomAt(verticesB[j])->getLocation() );//The location retrieved from an atom is the actual pointer don't delete it
	 ...
	 if(!Settings::Sampling::short_range_sampling  ){
		 for(size_t i = 0; i<this->contacts.size(); i++) //between the contacts in helixA and helixB
		 {
			...
			this->edge_length[v2][v1] = df->sumOfRadii( getAtom(v1), getAtom(v2) ) ;//+ Settings::Collision::contactUpperThreshold ; //  *Settings::Collision::contactLowerThreshold ;//
...
bool ConvexChart::initializeChart( bool contin, ActiveConstraintRegion * region )
{
	...
	for(int i = this->parameters.size()-1; i>=0; i--){ //to start from reverse if you put contacts at the end
		...
		if(  isContact >= 0 && Settings::Sampling::short_range_sampling  ){
				 this->param_lengthUpper[v1][v2] = df->bondingUpperBound( getAtom(v1), getAtom(v2) );
				 this->param_lengthLower[v1][v2] = df->bondingLowerBound( getAtom(v1), getAtom(v2) );
		 }
		 else
		 {
...
//			else       //COMMENDED BECAUSE i ADD desc->setWitnessPoint(wp4); TO ALGORITHM
//				this->witnessPoint[i] = (*(this->currentGraph->getSpace().front()))[i]; //start from the middle(witness) point
			...
			if (contin) {
				if (region->getCombinedSize() > 0) //space + witness not empty  //getSpace().empty()
				{
					...
					if (region->getWitnessSize() > 0)
						this->witnessPoint[i]	= (*(region->getWitness().front()))[i]; //start from the middle(witness) point
				} else
		...
		if(this->witnessPoint[i] < this->param_lengthLower[v1][v2] && this->witnessPoint[i] > this->param_lengthLower[v1][v2]- 0.1   ){
			this->witnessPoint[i] = this->param_lengthLower[v1][v2];
		}
		else if(this->witnessPoint[i] > this->param_lengthUpper[v1][v2] && this->witnessPoint[i] < this->param_lengthUpper[v1][v2]+ 0.1   ){
			this->witnessPoint[i] = this->param_lengthUpper[v1][v2];
		}

		if(this->witnessPoint[i] < this->param_lengthLower[v1][v2] || this->witnessPoint[i]> this->param_lengthUpper[v1][v2]  ){ //quick fix and for debug purpose
				cout <<"ERRORRRRRRRRs  witnessPoint is out of boundary "<< this->witnessPoint[i]  <<  " " << this->witnessPoint[i]-this->param_lengthLower[v1][v2] << " " << this->param_lengthUpper[v1][v2]-this->witnessPoint[i]  <<endl;    //one reason is; when a parameter becomes a contact, its value is set to be sum of radius. since it is forced to be exact contact, the range for the new parameters may not fit the value of witness orientation.
				this->witnessPoint[i] = this->param_lengthLower[v1][v2];
		...
		//-------added to test jacobian, it finds better starting point somehow when it starts from the middle
		if(Settings::Sampling::JacobianSampling)
		{
				this->witnessPoint[i] = ( this->param_lengthLower[v1][v2] + this->param_lengthUpper[v1][v2] )/2;
				this->point[i] = this->witnessPoint[i] ;
		}
		//-------
	...
	for(int i = 0;i<NO;i++)
		for(int j = i; j<NO; j++){
			if(this->param_lengthLower[i][j] > this->param_lengthUpper[i][j]){
				cout << "invalid description along parameter " << i << "-" << j << " min " << this->param_lengthLower[i][j] << " max " << this->param_lengthUpper[i][j]  << endl;
				//return false;  //commented on 25oct11 since i arrange the range according to param_length, it is possible that just current value of param_length is blue
			}
	}
...
void ConvexChart::setRangeByTriangleInequality(int v1, int v2){
	...
	if( this->edge_length[v1][v2] == -1 || std::isnan(this->edge_length[v1][v2])   ){
		double max = 100;
		for(int i=0; i<NO; i++){

			int paramv1iNo = find_pair_in_vector( this->parameters, make_pair(v1, i));  // pair = <v, i> is not ordered
			int paramv2iNo = find_pair_in_vector( this->parameters, make_pair(v2, i));
			if((paramv1iNo>=0 && paramv1iNo<paramv1v2No) || (paramv2iNo>=0 && paramv2iNo<paramv1v2No)  ) // if v1-i is a parameter that is not set yet, then skip. order of the parameter is important.
		...
		for(int i=0; i<NO; i++){
			...
			int paramv2iNo = find_pair_in_vector( this->parameters, make_pair(v2, i));
			if((paramv1iNo>=0 && paramv1iNo<paramv1v2No) || (paramv2iNo>=0 && paramv2iNo<paramv1v2No)  )
				continue;
...
//----------------
			...
			else localmin = 0;  //otherwise min and max ranges are intermingled, hence at the intersected places the difference of the edges is 0
//----------------
...
int ConvexChart::find_pair_in_vector(vector<pair<int,int> > ivector, pair<int,int> ipair)
{
	int index = -1;
	for(vector<pair<int,int> >::iterator iter = ivector.begin(); iter != ivector.end(); iter++)
	{
		index++;
		if( ((*iter).first == ipair.first &&  (*iter).second == ipair.second ) || ((*iter).first == ipair.second &&  (*iter).second == ipair.first ) )
			return index;
	}
	return -1;
}
...
{
	double mid = (this->param_lengthUpper[v1][v2] + this->param_lengthLower[v1][v2])/2;
	double ratio =  (mid-this->point[p])/(this->param_lengthUpper[v1][v2] -this->param_lengthLower[v1][v2]);
...
void ConvexChart::stepGrid(){
	...
	if(dense)
	{
		...
		{
			double x = (this->point[i] - this->witnessPoint[i])/ (2*this->stepSize);
			int y = ceil(x); x= x-y;
	...
	for(size_t i = 0; i<this->parameters.size(); i++){
		if(carry )
		{
			...
			if(this->point[p] > this->param_lengthUpper[v1][v2] && this->point[p] < temp ){
			...
			if(this->point[p] < this->param_lengthLower[v1][v2] && this->point[p] > temp){
...
//				if(tmpoint < this->edge_lengthLower[v1][v2]) //minconnection is changing since i compute the range continuously
//					this->point[p] = this->edge_lengthLower[v1][v2];
//				if(tmpoint > this->edge_lengthUpper[v1][v2]) //maxconnection is changing since i compute the range continuously
//					this->point[p] = this->edge_lengthUpper[v1][v2];
	...
	for(int x = p; x>=0; x--){
		...
		//todo then why do you have 2 for loops and updateList, the first loop would be sufficient?!!
		//not actually! wouldnot be sufficient. updating xth parameter may not require the range computation for x-1th param, so if you do not check dependency you may do extra work, for instance  a->b and a->c->d case changing b doesnot require update on c
		//however if there is a circular type of dependency, for example 3 non-edges in one tetrahedra, like a->b->c and a->c
		//then changing 'a' requires updates on the range of b and c, then changing the param value of b requires updates on c.
		//As a result the range of c will be computed twice! bad.
...
//todo: i think those 2 for loops should merged, because when xth param value changed to exact mid value, the boundary of the parameters less than x, will change again.
...
void ConvexChart::computeRange(int v1, int v2, int paramNo)
{
	if( boundaryComputationWay[paramNo] == -1)
		...
		setRangeByTetrahedralInequality(v1, v2, boundaryComputationWay[paramNo]);//this method not only sets min, max but also param ! check if that is what u want !!
}
...
bool ConvexChart::stepAround(){
	...
	if(dir ==  2*this->parameters.size() -1)
		...
		{
			stepSize_of_stepGridContact = this->stepSize - (this->point[dir/2] - this->param_lengthUpper[v1][v2]);
...
void ConvexChart::stepGridBinary(bool valid){
	...
	if(dir%2 == 0 )
		this->point[dir/2] -= times * stepSize_of_stepGridContact / pow(2., bin);   //forward direction
	else
		this->point[dir/2] += times * stepSize_of_stepGridContact / pow(2., bin);   //backward direction
...
void ConvexChart::setRangeByTetrahedralInequality(int v1, int v2, int tetraNo)
{
	...
	int no=0;
	for(int m=0; m<4; m++)
		if( this->tetrahedra[tetraNo][m] != v1 && this->tetrahedra[tetraNo][m] != v2 )
		{
	...
	inds[3]=v2;
		...
		if(param_length[ inds[0] ][ inds[1]] == -1 ||  param_length[ inds[0] ][ inds[2]] == -1 || param_length[ inds[1] ][ inds[2]] == -1 || param_length[ inds[0] ][ inds[3]] == -1 || param_length[ inds[1] ][ inds[3]] == -1 )
			...
			double posi[4][3];
			posi[0][0] = 0;	posi[0][1] = 0; 	posi[0][2] = 0;
			posi[1][0] = param_length[ inds[0] ][ inds[1]]; 	posi[1][1] = 0; 	posi[1][2] = 0;

			double ac= param_length[ inds[0] ][ inds[2] ], ab= param_length[ inds[0] ][ inds[1] ], bc=param_length[ inds[1] ][ inds[2] ];
			double cosCAB =  (ac*ac + ab*ab - bc*bc)/ (2*ac*ab) ;  double sinCAB = sqrt( abs(1-cosCAB*cosCAB) );
			posi[2][0] = param_length[ inds[0] ][ inds[2] ]*cosCAB; 	posi[2][1] = param_length[ inds[0] ][ inds[2] ]*sinCAB; 	posi[2][2] = 0;

			ac= param_length[ inds[0] ][ inds[3] ]; ab= param_length[ inds[0] ][ inds[1] ]; bc=param_length[ inds[1] ][ inds[3] ];
			cosCAB =  (ac*ac + ab*ab - bc*bc)/ (2*ac*ab) ;  sinCAB = sqrt( abs(1-cosCAB*cosCAB) );
			posi[3][0] = param_length[ inds[0] ][ inds[3] ]*cosCAB; 	posi[3][1] = param_length[ inds[0] ][ inds[3] ]*sinCAB; 	posi[3][2] = 0;

			double nposi[2]; nposi[0] = posi[3][0]; nposi[1] = -1 * posi[3][1];
			double minrange = sqrt( (posi[2][0]-posi[3][0])*(posi[2][0]-posi[3][0]) + (posi[2][1]-posi[3][1])*(posi[2][1]-posi[3][1])  );
			double maxrange = sqrt( (posi[2][0]-nposi[0])*(posi[2][0]-nposi[0]) + (posi[2][1]-nposi[1])*(posi[2][1]-nposi[1])  );
