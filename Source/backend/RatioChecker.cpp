...
//RatioChecker::RatioChecker(SaveLoader* snl, MolecularUnit  *hA, MolecularUnit *hB, Atlas *mapview)
...
//void RatioChecker::preComputeCache(DistinctRealization & correct_dr, vector<AtlasNode*> nodes, PredefinedInteractions *df)
//{
//    // pre-compute all relizations
//    RatioBookkeeper tempbk(300, RealizationHash(helxA, correct_dr), RealizationCompare(helxA));
...
//                        DistinctRealization dr(this->helxA, df, fa, ta, fb, tb, node, spacenum);
...
//                region->trim();
//            }
//            for(RatioBookkeeper::iterator iter = tempbk.begin();iter != tempbk.end();iter++){
...
//			getDistanceFinderFromFile("tt.txt"); //,pair<double,double>(0.4,0.4)
...
//	//DistinctRealization correct_dr(this->helxA, vector<double>(correct_mat,correct_mat+12),0,0);
//	DistinctRealization correct_dr(this->helxA, distfinder, Utils::getMatrixFromFileFromTo("fromto.txt"),0,0);
//	RealizationCompare rc(this->helxA,0.1);
//	bookkeeper = new RatioBookkeeper(300,RealizationHash(helxA,correct_dr),RealizationCompare(helxA));
...
//    for(vector<vector<pair<int,int> > >::iterator pit = exclu_pairs.begin();pit != exclu_pairs.end();pit++){
//        for(int i = 0;i < pit->size();i++){
//            cout << "dropping constrain " << pit->at(i).first << "---" << pit->at(i).second << endl;
//        }
//        bookkeeper->clear();
...
//            int inode;
//            linestr >> inode;
//            vector<pair<int,int> > parts = (*mapView)[inode]->getCG()->getParticipants();
//            bool found = false;
//            for(int i = 0;i < pit->size();i++){
//                if(find(parts.begin(), parts.end(), pit->at(i)) != parts.end()){
//                    found = true;
//                    break;
//                }
//            }
...
//                }
//                DistinctRealization dr(this->helxA, distfinder, mat, inode, 0, &fstring);
...
//        int true_copies = 0;
//        int num_collision = 0;
//        for(RatioBookkeeper::iterator iter = bookkeeper->begin();iter != bookkeeper->end();iter++){
//            // gather some infomation
//            all_copies += iter->second;
//            num_collision += iter->first.num_collision;
//            // print to file
//            copies_file << iter->first.node << "," << iter->first.spacenum << "," << iter->second << "," << rc.difference(iter->first, correct_dr);
//            copies_file << ",[";
...
//                cout << "correct one ";
//                cout << iter->first.node << "," << iter->first.spacenum << "," << iter->second << "," << rc.difference(iter->first, correct_dr) << ": ";
...
//            bool fit = true;
//            for(PredefinedInteractions::dist_iterator dis = distfinder->dist2begin();dis != distfinder->dist2end();dis++){
...
//                double real_dist = sqrt((Vector(ata->getLocation()) - Vector(atb->getLocation()).trans(iter->first.trans_mat)).squared_length());
...
//        cout << "all true distinct zero-dim realizations " << true_distinct << endl; // all true distinct zero-dim realizations
//        cout << "num of everage collisions" << (double)((num_collision)) /all_distinct << endl;
...
//PredefinedInteractions* RatioChecker::getDistanceFinderFromFile(const char* filename){
