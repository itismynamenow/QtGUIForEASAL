...
ActiveConstraintGraph::ActiveConstraintGraph(ActiveConstraintGraph* ptrToCpy){
	...
	for(vector<pair<int,int> >::iterator iter = ptrToCpy->participants.begin();iter != ptrToCpy->participants.end();iter++) //why not just copy all from prtToCpy
	{
		pair<int,int> temp;
		...
		if( find(this->unordered_setA.begin(), this->unordered_setA.end(), temp.first) == this->unordered_setA.end() )
			this->unordered_setA.push_back((*iter).first);
		if( find(this->unordered_setB.begin(), this->unordered_setB.end(), temp.second) == this->unordered_setB.end() )
...
ActiveConstraintGraph::ActiveConstraintGraph(vector<pair<int,int> > &participants, MolecularUnit* helA, MolecularUnit* helB )
{
	...
	for (vector<pair<int,int> >::iterator iter = participants.begin();
		iter != participants.end();
		iter++)
	{
		this->participants.push_back(*iter);

		//first appeared contact is first added to the set
		if( find(this->unordered_setA.begin(), this->unordered_setA.end(), (*iter).first) == this->unordered_setA.end() )
			this->unordered_setA.push_back((*iter).first);
		if( find(this->unordered_setB.begin(), this->unordered_setB.end(), (*iter).second) == this->unordered_setB.end() )
			this->unordered_setB.push_back((*iter).second);
	}
	//	this->unordered_setX.unique(); //unique does not work since it removes 'consecutive' equal elements. Hence you need to check existence before adding as it is done above.
...
ActiveConstraintGraph::ActiveConstraintGraph(vector<pair<int,int> > &participants, MolecularUnit* helA, MolecularUnit* helB, vector<pair<int,int> > PL)
{
	this->stepSize = Settings::Sampling::stepSize;
	...
	this->verticesA.assign( this->unordered_setA.begin(),  this->unordered_setA.end() );
	this->verticesB.assign( this->unordered_setB.begin(),  this->unordered_setB.end() );

	// the vertex that 'first' appear in PL should be added to verticesA/B 'first'.
	for (vector<pair<int,int> >::iterator it = PL.begin();
		it != PL.end();
		it++)
	{
		if( find(this->verticesA.begin(), this->verticesA.end(), (*it).first) == this->verticesA.end() )
			this->verticesA.push_back( (*it).first );

		if( find(this->verticesB.begin(), this->verticesB.end(), (*it).second ) == this->verticesB.end() )
...
{
	this->contacts.clear();
	for(vector<pair<int,int> >::iterator iter = this->participants.begin(); iter != this->participants.end(); iter++)
	{
		pair<int,int> temp;
...
{
	this->parameters.clear();
	for(vector<pair<int,int> >::iterator iter = this->paramlines.begin(); iter != this->paramlines.end(); iter++){
...
void ActiveConstraintGraph::completeTo3by3Graph()
{
		...
		if( this->unordered_setA.size() <= 2 )  //find missing vertices
		{
			vector<int> toAdd;
			if(closestAtom)
				toAdd =  completeTo3by3Graph_byClosestAtom(helA, this->verticesA, this->unordered_setA.size() );
			else //perpendicular angle
				toAdd =  completeTo3by3Graph_that_makes_perpendicularAngle(helA, this->verticesA, this->unordered_setA.size() );
		...
		if( this->unordered_setB.size() <= 2 ) //find missing vertices
		{
			vector<int> toAdd;
			if(closestAtom)
				toAdd =  completeTo3by3Graph_byClosestAtom(helB, this->verticesB, this->unordered_setB.size() );
			else //perpendicular angle
				toAdd =  completeTo3by3Graph_that_makes_perpendicularAngle(helB, this->verticesB, this->unordered_setB.size() );
...
vector<int> ActiveConstraintGraph::completeTo3by3Graph_byClosestAtom( MolecularUnit * hel, vector<int> vertices, int size )
{
...
vector<int> ActiveConstraintGraph::completeTo3by3Graph_that_makes_perpendicularAngle(MolecularUnit * hel, vector<int> vertices, int size)
{
		vector<int> out_vertices;
		...
		if( size == 1 ) //Find 2nd atom
		{
			...
			for(int h=0; h<hel_size; h++)
			{
				if(h == vertices[0])
					continue;
				double dist = Utils::dist(hel->getAtomAt(vertices[0])->getLocation(), hel->getAtomAt(h)->getLocation() );
				if(dist <= minDist)
				{
					vertices_1 = h;
					minDist = dist;
			...
			if(vertices_1 == -1) cout << "ERROR in completeTo3by3Graph_that_makes_perpendicularAngle " << endl;
		...
		for(int h = 0; h < hel_size; h++){
			if(vertices[0]== h ||  vertices_1 == h)   // if h already exists in current vertices
				continue;
			double dist_01 = Utils::dist( hel->getAtomAt(vertices[0])->getLocation(), hel->getAtomAt(vertices[1])->getLocation() );
			double dist_0h = Utils::dist( hel->getAtomAt(vertices[0])->getLocation(), hel->getAtomAt(h)->getLocation() );
			double dist_1h = Utils::dist( hel->getAtomAt(vertices[1])->getLocation(), hel->getAtomAt(h)->getLocation() );
			double angle = acos( (dist_0h*dist_0h + dist_1h*dist_1h - dist_01*dist_01) / (2*dist_0h*dist_1h) )  * 180.0 / PI;
...
int ActiveConstraintGraph::getA(int no)
{
	if( this->verticesA.size() > no)
		...
		cout << "getA ERROR no:" << no << " this->verticesA.size() " << this->verticesA.size()  << endl;
		return -1;
	}
}
...
int ActiveConstraintGraph::getB(int no)
{
	if( this->verticesB.size() > no)
		...
		cout << "getB ERROR" << endl<< no << " this->verticesB.size() " << this->verticesB.size()  << endl;
		return -1;
	}
}
...
int ActiveConstraintGraph::find_in_setA(int atom){
	...
	int i = 0;
	for(list<int>::iterator iter = this->unordered_setA.begin();  iter != this->unordered_setA.end(); iter++)
	{
		if(*iter == atom)
			return i;
		i++;
...
int ActiveConstraintGraph::find_in_setB(int atom){
	...
	int i = 0;
	for(list<int>::iterator iter = this->unordered_setB.begin();  iter != this->unordered_setB.end(); iter++)
	{
		if(*iter == atom)
			return i;
		i++;
...
int ActiveConstraintGraph::find_in_verticesA(int atom){
	...
	int i = 0;
	for(vector<int>::iterator iter = this->verticesA.begin();  iter != this->verticesA.end(); iter++)
	{
		if(*iter == atom)
			return i;
		i++;
...
int ActiveConstraintGraph::find_in_verticesB(int atom){
	...
	int i = 0;
	for(vector<int>::iterator iter = this->verticesB.begin();  iter != this->verticesB.end(); iter++)
	{
		if(*iter == atom)
			return i;
		i++;
...
bool ActiveConstraintGraph::mayHaveThirdAtom(){
	...
	if(this->participants.size() < 6 || ( this->unordered_setA.size() > 2  &&  this->unordered_setB.size() > 2 )    )
		return true;
	return false;
}
...
void ActiveConstraintGraph::addContact(pair<int,int> contact){
		...
		//first appeared contact is first added to the set
		if( find(this->unordered_setA.begin(), this->unordered_setA.end(), contact.first) == this->unordered_setA.end() ) //if cannot find, then push
			this->unordered_setA.push_back(contact.first);
		if( find(this->unordered_setB.begin(), this->unordered_setB.end(), contact.second) == this->unordered_setB.end() )
...
bool ActiveConstraintGraph::IsParentOf(ActiveConstraintGraph* other){  //if other is child of this, then it should have more contacts
	...
	if(output){ //checks if all contacts of this included in other.
		for(vector<pair<int,int> >::iterator iter = this->participants.begin();  output && iter != this->participants.end();  iter++){
			bool found = false;
			for(vector<pair<int,int> >::iterator iter2 = other->participants.begin();   !found && iter2 != other->participants.end();  iter2++){
				if((*iter).first == (*iter2).first && (*iter).second == (*iter2).second){
					found = true;
				}
			}
			if(!found) output = false; // this CG has a different contact pair than otherCG.
		}
	}


	output = output && (this->helA == other->getMolecularUnitA() ); //make it slow?  // i put this here from top, to make it not checked if participants do not match
	output = output && (this->helB == other->getMolecularUnitB() );  //if it checks just pointer address then it is not slow
	return output;
}
...
bool ActiveConstraintGraph::isConnected(int x, int y){
	bool output = false;
	for(vector<pair<int,int> >::iterator it = this->participants.begin();it!= this->participants.end();it++){
...
ostream& operator<<(ostream& os, ActiveConstraintGraph &acg){
	...
	for(vector<pair<int,int> >::iterator iter = acg.participants.begin(); iter != acg.participants.end(); iter++)
		os<< (*iter).first << "-" << (*iter).second << "   ";
	os<<endl;


	os<< "unordered_setA" << endl;
	for(list<int>::iterator iter = acg.unordered_setA.begin(); iter != acg.unordered_setA.end(); iter++)
		os<< (*iter) << "   ";
	os<<endl;


	os<< "unordered_setB" << endl;
	for(list<int>::iterator iter = acg.unordered_setB.begin(); iter != acg.unordered_setB.end(); iter++)
		os<< (*iter) << "   ";
	os<<endl;


	os<< "verticesA" << endl;
	for(vector<int>::iterator iter = acg.verticesA.begin(); iter != acg.verticesA.end(); iter++)
		os<< (*iter) << "   ";
	os<<endl;


	os<< "verticesB" << endl;
	for(vector<int>::iterator iter = acg.verticesB.begin(); iter != acg.verticesB.end(); iter++)
		os<< (*iter) << "   ";
	os<<endl;
	...
	for(vector<pair<int,int> >::iterator iter = acg.contacts.begin();iter != acg.contacts.end();iter++)
		os << "c "<< iter->first << " " << iter->second << " " ;
	os << endl;


	os << "paramlines.size " << acg.paramlines.size() << " ";
	for(vector<pair<int,int> >::iterator iter = acg.paramlines.begin();iter != acg.paramlines.end();iter++)
		os << "p "<< iter->first << " " << iter->second << " " ;
	os << endl;


	os << "parameters.size " << acg.parameters.size() << " ";
	for(vector<pair<int,int> >::iterator iter = acg.parameters.begin();iter != acg.parameters.end();iter++)
		os << "p "<< iter->first << " " << iter->second << " " ;
	os << endl;
...
vector<ActiveConstraintGraph*> ActiveConstraintGraph::getAllAncestors(){
	...
	for(int i = 0; i < power_of_two; i++){
		...
		ActiveConstraintGraph* cgnew = new ActiveConstraintGraph(parts,this->helA,this->helB);
		// guaranteed >= 0 since it returns unsigned (size_t)
		if(cgnew->getDim() <= Settings::RootNodeCreation::dimension_of_rootNodes){ // && cgnew->getDim() >= 0
