...
public:
	 ...
	 */
	static void createChildNodes(AtlasNode *rnode_prime, std::list<std::pair<int,int> > contactList, Orientation* ori ,  Atlas* atlas,  SaveLoader* snl, bool firstPath=true);
...
public:
	 ...
	 */
	AtlasBuilder(MolecularUnit* a, MolecularUnit* b,SaveLoader* snl, PredefinedInteractions *df, Atlas* atlas );
    AtlasBuilder();

    /** @brief Destructor */
	virtual ~AtlasBuilder();

    void setData(MolecularUnit *helA, MolecularUnit *helB, SaveLoader *snl, PredefinedInteractions *df, Atlas *atlas);
	 ...
	 */
	bool sampleTheNode(AtlasNode *rnode, bool dense, Orientation* coming_witness_ori, bool continu, bool bret);

	/** @brief add witness point to all its parents if reverseWitness is enabled */
	void addReverseWitness(AtlasNode *rnode, ActiveConstraintGraph *cgK, ConvexChart *chart, ActiveConstraintRegion *region, Orientation* coming_witness_ori, bool & noGoodOrientation);

	virtual bool MyJacobianSampleRec(AtlasNode *rnode, bool dense, Orientation* orrw, bool continu, bool breath, bool bret ){return false;};
	virtual bool MyJacobianSample(AtlasNode *rnode, bool dense, Orientation* orrw, bool continu, bool breath, bool bret ){return false;};

    static void refigureFlip(Orientation* orr, std::vector< std::vector<int> > flipScheme, SaveLoader* snlr);
...
protected:
	 ...
	 */
    //std::list<Orientation*> findRealizations(ActiveConstraintGraph *cgK, ConvexChart* des);
    std::list<Orientation*> findRealizations(ActiveConstraintGraph *cgK, ConvexChart* des, vector<vector<int> > flipScheme);
	 ...
	 */
	void determineStepSizeDynamically(ActiveConstraintGraph *cgK, ActiveConstraintRegion * region, bool dense, CayleyParameterization * cparam  );
	 ...
	 */
	void findBoundary(std::list<Orientation*>::iterator ori_on_lattice, ConvexChart* desc,
            ...
            int& noPoints, bool& boundary_ori_found_and_saved, CayleyPoint* p4d);
	 ...
	 */
	void createChildContactGraps_fromTheBoundary(std::list<std::pair<int, int> >& contactList,
			std::list<Orientation*>::iterator ori_on_lattice, ActiveConstraintGraph* cgK,
	...
	AtlasNode* createChildNode(AtlasNode *rnode, ActiveConstraintGraph *cgKprime,
			Orientation* wit_orr_toSend, bool & savedOnce, bool leafWitness,
